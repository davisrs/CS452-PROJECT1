...
GLuint vaoBox,vaoPlatform;
GLuint vboBox,vboPlatform;
...
void render() {
	...
	glDrawElements (GL_TRIANGLES, num_indicesBox, GL_UNSIGNED_INT, NULL);
	fprintf(stderr, "HERE\n");
	glutSwapBuffers();
	glutPostRedisplay();
...
/* ---------------------------------------- for the cube ----------------------------------------*/
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(num_indicesBox), squareIndices, GL_STATIC_DRAW);
	...
	// Create the buffer, but don't load anything yet
	glBufferData(GL_ARRAY_BUFFER, sizeof(platformColors)+sizeof(platformVertices), NULL, GL_STATIC_DRAW);
	// Load the vertex points
	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(platformVertices), platformVertices); 
	// Load the colors right after that
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(platformVertices),sizeof(platformColors), platformColors);

	glGenBuffers(1, &indexBufferID);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexBufferID);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, num_indicesPlatform*sizeof(GLuint), platformIndices, GL_STATIC_DRAW);
	
	glVertexAttribPointer(positionID, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glVertexAttribPointer(colorID, 3, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(sizeof(platformVertices)));
	*/
